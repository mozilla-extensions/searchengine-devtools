...
export default class AllEnginesView extends HTMLElement {
  ...
  async loadEngines(event, config, attachmentBaseUrl, iconConfig) {
    ...
    let records = this.#config.data.sort(sortRecordsByIdentifier);
  ...
  #findApplications(variants) {
    ...
    for (let variant of variants) {
      if (!variant?.environment?.applications) {
        // If we don't have applications specified, then we'll be available to all.
        return ["all"];
      }
      // Otherwise figure out if the engine is available to all or not.
      for (let application of variant?.environment?.applications ?? []) {
  ...
  /**
   * Insert deployments for a record.
